...
struct Literal {
    ...
    consteval Literal(const char (&s)[N]) { std::copy(s, s + N, &data[0]); }
};
...
struct Regex {
    /*
            ...
            otherwise: the class takes a valid regular expression, and stores it for persistent
       view.
